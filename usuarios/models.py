...
class UserManager(BaseUserManager):
    ...
    def create_superuser(self, email, password=None, **extra_fields):
        ...
        extra_fields.setdefault(
            "email_verified", True
        )  # Los superusuarios están verificados por defecto
...
class Usuario(AbstractUser):
    ...
    verification_token = models.UUIDField(
        blank=True, null=True, help_text="Token para verificación de email"
    )
    verification_token_expires = models.DateTimeField(
        blank=True, null=True, help_text="Fecha de expiración del token"
    )
    ...
    password_reset_token_expires = models.DateTimeField(
        blank=True, null=True, help_text="Fecha de expiración del token de recuperación"
    )
    ...
    last_password_reset_attempt = models.DateTimeField(
        blank=True, null=True, help_text="Fecha del último intento de recuperación"
    )
    ...
    def generate_password_reset_token(self):
        ...
        if not self.can_request_password_reset():
            raise ValueError(
                "Has excedido el límite de intentos de recuperación. Intenta en 24 horas."
            )
        ...
        now = timezone.now()
        if (
            not self.last_password_reset_attempt
            or now - self.last_password_reset_attempt > timedelta(hours=24)
        ):
...
class Interesado(models.Model):
    ...
    usuario = models.OneToOneField(
        Usuario, on_delete=models.CASCADE, related_name="interesado"
    )
    ...
    estado_id = models.IntegerField(
        blank=True, null=True, help_text="ID del estado desde catálogo"
    )
    municipio_id = models.IntegerField(
        blank=True, null=True, help_text="ID del municipio desde catálogo"
    )
    localidad_id = models.IntegerField(
        blank=True, null=True, help_text="ID de la localidad desde catálogo"
    )
    calle_numero = models.CharField(
        max_length=200, blank=True, null=True, verbose_name="Calle y número"
    )
    ...
    def save(self, *args, **kwargs):
        try:
            ...
            if (
                old_instance.foto_perfil
                and old_instance.foto_perfil != self.foto_perfil
            ):
    ...
    def __str__(self):
        if self.nombre and self.apellido_paterno:
            apellido_completo = (
                f"{self.apellido_paterno} {self.apellido_materno}"
                if self.apellido_materno
                else self.apellido_paterno
            )
    ...
    def actualizar_ubicacion_desde_catalogo(self, localidad_obj):
        ...
        if localidad_obj:
            ...
            if localidad_obj.catalogo_codigo_postal:
                self.codigo_postal = str(
                    localidad_obj.catalogo_codigo_postal.codigo_postal
                )
    ...
    def tiene_cv_completo(self):
        ...
        return (
            hasattr(self, "curriculum")
            and self.nombre
            and self.apellido_paterno
            and self.curriculum.resumen_profesional
            and self.codigo_postal  # Ahora también requiere código postal
        )
...
class Reclutador(models.Model):
    usuario = models.OneToOneField(
        Usuario, on_delete=models.CASCADE, related_name="reclutador"
    )
    secretaria = models.ForeignKey(
        Secretaria, on_delete=models.CASCADE, related_name="reclutadores"
    )
...
class Vacante(models.Model):
    ...
    # Información básica
    secretaria = models.ForeignKey(
        Secretaria, on_delete=models.CASCADE, related_name="vacantes"
    )
    reclutador = models.ForeignKey(
        Reclutador, on_delete=models.CASCADE, related_name="vacantes"
    )
    titulo = models.CharField(max_length=200)
    descripcion = models.TextField()
    categoria = models.ForeignKey(
        Categoria, on_delete=models.CASCADE, related_name="vacantes"
    )
    ...
    municipio = models.CharField(
        max_length=50, choices=MUNICIPIOS_ESTADO_MEXICO, verbose_name="Municipio"
    )
    ...
    detalles_salario = models.CharField(
        max_length=200,
        blank=True,
        null=True,
    ...
    estado_vacante = models.CharField(
        max_length=15, choices=ESTADOS_VACANTE, default="borrador"
    )
    aprobada = models.BooleanField(default=False)
    destacada = models.BooleanField(default=False)

    # Límites de postulación
    max_postulantes = models.IntegerField(
    ...
    def cerrar_vacantes_vencidas(cls):
        ...
        vacantes_vencidas = cls.objects.filter(
            estado_vacante="publicada", fecha_limite__lt=hoy
        )
...
class RequisitoVacante(models.Model):
    ...
    vacante = models.OneToOneField(
        Vacante, on_delete=models.CASCADE, related_name="requisitos"
    )
    ...
    def experiencia_display(self):
        ...
        if self.experiencia_minima:
            return dict(self.EXPERIENCIA_CHOICES).get(
                self.experiencia_minima, self.experiencia_minima
            )
...
class Curriculum(models.Model):
    ...
    interesado = models.OneToOneField(
        Interesado, on_delete=models.CASCADE, related_name="curriculum"
    )
...
class ExperienciaLaboral(models.Model):
    ...
    curriculum = models.ForeignKey(
        Curriculum, on_delete=models.CASCADE, related_name="experiencias"
    )
    empresa = models.CharField(max_length=200)
    puesto = models.CharField(max_length=200)
    descripcion = models.TextField(
        help_text="Funciones y responsabilidades principales"
    )
    fecha_inicio = models.DateField()
    fecha_fin = models.DateField(blank=True, null=True)
    actual = models.BooleanField(
        default=False, help_text="Marca si es tu trabajo actual"
    )
...
class Educacion(models.Model):
    ...
    curriculum = models.ForeignKey(
        Curriculum, on_delete=models.CASCADE, related_name="educaciones"
    )
    titulo = models.CharField(
        max_length=200, help_text="Título obtenido o nivel educativo"
    )
    institucion = models.CharField(max_length=200)
    fecha_inicio = models.DateField()
    fecha_fin = models.DateField(blank=True, null=True)
    descripcion = models.TextField(
        blank=True, null=True, help_text="Descripción adicional"
    )
...
class HabilidadInteresado(models.Model):
    ...
    curriculum = models.ForeignKey(
        Curriculum, on_delete=models.CASCADE, related_name="habilidades"
    )
...
class IdiomaInteresado(models.Model):
    ...
    curriculum = models.ForeignKey(
        Curriculum, on_delete=models.CASCADE, related_name="idiomas"
    )
...
class Postulacion(models.Model):
    ...
    interesado = models.ForeignKey(
        Interesado, on_delete=models.CASCADE, related_name="postulaciones"
    )
    vacante = models.ForeignKey(
        Vacante, on_delete=models.CASCADE, related_name="postulaciones"
    )
    curriculum = models.ForeignKey(
        Curriculum, on_delete=models.CASCADE, related_name="postulaciones"
    )
    fecha_postulacion = models.DateTimeField(auto_now_add=True)
    estado = models.CharField(
        max_length=20, choices=ESTADOS_POSTULACION, default="enviada"
    )
    mensaje_motivacion = models.TextField(
        blank=True, null=True, help_text="Mensaje opcional del candidato"
    )
    notas_reclutador = models.TextField(
        blank=True, null=True, help_text="Notas del reclutador"
    )
    fecha_actualizacion = models.DateTimeField(auto_now=True)
    ...
    class Meta:
        ...
        unique_together = [
            "interesado",
            "vacante",
        ]  # Un interesado solo puede postularse una vez por vacante
...
class Municipios(models.Model):
    ...
    catalogo_delegacion = models.ForeignKey(
        "Delegaciones", on_delete=models.SET_NULL, null=True, blank=True
    )
...
class Localidades(models.Model):
    ...
    catalogo_codigo_postal = models.ForeignKey(
        "Codigos_Postales", on_delete=models.PROTECT
    )
    # Este no va
    catalogo_tipo_asentamiento = models.ForeignKey(
        "Tipos_Asentamientos", on_delete=models.PROTECT
    )
    catalogo_zona_regional = models.ForeignKey(
        "Zonas_Regionales", on_delete=models.PROTECT
    )
