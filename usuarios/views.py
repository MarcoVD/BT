...
def extend_session_ajax(request):
    ...
    try:
        ...
        if not request.headers.get("Content-Type") == "application/json":
            return JsonResponse(
                {"success": False, "error": "Petición inválida"}, status=400
            )
        ...
        try:
            ...
            logger.warning(
                f"No se pudo actualizar ultimo_acceso para usuario {request.user.id}: {e}"
            )

        # Log de la acción
        logger.info(
            f"Sesión extendida para usuario {request.user.email} ({request.user.id})"
        )
...
def session_status_ajax(request):
    ...
    try:
            ...
            session_info.update(
                {
                    "user": {
                        "email": request.user.email,
                        "rol": request.user.rol,
                        "ultimo_acceso": request.user.ultimo_acceso.isoformat()
                        if request.user.ultimo_acceso
                        else None,
                        "fecha_registro": request.user.fecha_registro.isoformat()
                        if hasattr(request.user, "fecha_registro")
                        else None,
                    }
                }
            )
        ...
        return JsonResponse(
            {"success": False, "error": "Error al obtener estado de sesión"}, status=500
        )
        ...
        if request.user.is_authenticated:
            ...
            if last_activity:
                ...
                if now - last_activity > self.session_timeout:
                    ...
                    logout(request)
                    logger.info(
                        f"Sesión cerrada automáticamente por timeout para usuario {request.user.email}"
                    )
...
def agregar_habilidad_ajax(request):
    if request.method != "POST":
        return JsonResponse(
            {"success": False, "error": "Método no permitido. Solo se acepta POST."}
        )
    ...
    try:
        ...
        if not hasattr(interesado, "curriculum"):
            return JsonResponse(
                {"success": False, "error": "Primero debes crear tu curriculum."}
            )
        ...
        if not nombre_habilidad:
            return JsonResponse(
                {"success": False, "error": "El nombre de la habilidad es requerido."}
            )

        if not nivel:
            return JsonResponse(
                {"success": False, "error": "El nivel de dominio es requerido."}
            )
        ...
        habilidad_existente = HabilidadInteresado.objects.filter(
            curriculum=curriculum, habilidad__nombre=nombre_habilidad
        ).first()
        ...
        habilidad_catalogo, created = Habilidad.objects.get_or_create(
            nombre=nombre_habilidad, defaults={"descripcion": f"{nombre_habilidad}"}
        )

        # Crear la relación entre el curriculum y la habilidad con su nivel
        habilidad_interesado = HabilidadInteresado.objects.create(
            curriculum=curriculum, habilidad=habilidad_catalogo, nivel=nivel
        )
...
def eliminar_habilidad_ajax(request, habilidad_id):
    ...
    if request.method != "DELETE":
        return JsonResponse(
            {"success": False, "error": "Método no permitido. Solo se acepta DELETE."}
        )
    ...
    try:
        ...
        if not hasattr(interesado, "curriculum"):
            return JsonResponse(
                {"success": False, "error": "No tienes un curriculum creado."}
            )
        ...
        habilidad_interesado = get_object_or_404(
            HabilidadInteresado, id=habilidad_id, curriculum=curriculum
        )
        ...
        return JsonResponse(
            {"success": False, "error": f"Error al eliminar la habilidad: {str(e)}"}
        )
...
def send_password_reset_email(request, user):
    ...
    try:
        ...
        reset_url = request.build_absolute_uri(
            reverse(
                "restablecer_contrasena", kwargs={"token": user.password_reset_token}
            )
        )
        ...
        html_message = render_to_string(
            "emails/recuperar_contrasena_email.html", context
        )
        ...
        try:
            ...
            if form.is_valid():
                try:
                    ...
                    # Log de la acción
                    logger.info(
                        f"Contraseña restablecida exitosamente para usuario {user.email}"
                    )
                    ...
                    logger.error(
                        f"Error al guardar nueva contraseña para {user.email}: {str(save_error)}"
                    )
                    form.add_error(
                        "new_password1",
                        "Error al guardar la nueva contraseña. Inténtalo nuevamente.",
                    )
...
def cleanup_expired_tokens():
    ...
    expired_verification = Usuario.objects.filter(
        verification_token_expires__lt=now, verification_token__isnull=False
    )
    ...
    expired_reset = Usuario.objects.filter(
        password_reset_token_expires__lt=now, password_reset_token__isnull=False
    )
    reset_count = expired_reset.update(
        password_reset_token=None, password_reset_token_expires=None
    )
    ...
    attempts_count = old_attempts.update(password_reset_attempts=0)

    logger.info(
        f"Tokens limpiados: {verification_count} verificación, {reset_count} recuperación, {attempts_count} intentos reiniciados"
    )
...
def send_verification_email(request, user):
    try:
        ...
        if user.rol == "interesado":
            ...
            subject = (
                "Verifica tu cuenta de Reclutador - Bolsa de Trabajo Estado de México"
            )
    ...
    def get(self, request, token):
        ...
        try:
            ...
            if not user:
                ...
                return render(
                    request,
                    "usuarios/verificacion_resultado.html",
                    {
                        "success": False,
                        "title": "Token Inválido",
    ...
    def post(self, request):
        ...
        try:
            ...
            if user.email_verified:
                messages.info(
                    request, "Tu cuenta ya está verificada. Puedes iniciar sesión."
                )
    ...
    def post(self, request):
        ...
        if form.is_valid():
            try:
                ...
                return render(
                    request,
                    "usuarios/registro_exitoso.html",
                    {"user_email": user.email, "user_role": "interesado"},
                )

            except Exception as e:
                logger.error(f"Error en registro de interesado: {str(e)}")
                messages.error(
                    request, "Error al crear la cuenta. Inténtalo nuevamente."
                )
...
class ReclutadorRegistroView(View):
    ...
    def get(self, request):
        ...
        try:
            ...
            messages.error(
                request,
                "Error: La Secretaría de Movilidad no está configurada en el sistema.",
            )
            return redirect("index")

        return render(
            request,
            "usuarios/registro_reclutador.html",
            {
        ...
        if reclutador_form.is_valid():
            try:
                ...
                try:
                    ...
                    messages.error(
                        request,
                        "Error: La Secretaría de Movilidad no está configurada.",
                    )
                    return render(
                        request,
                        "usuarios/registro_reclutador.html",
                ...
                Reclutador.objects.create(
                    ...
                    apellido_paterno=reclutador_form.cleaned_data.get(
                        "apellido_paterno"
                    ),
                    apellido_materno=reclutador_form.cleaned_data.get(
                        "apellido_materno"
                    ),
                ...
                return render(
                    request,
                    "usuarios/registro_exitoso.html",
                    {"user_email": user.email, "user_role": "reclutador"},
                )
    ...
    try:
        ...
        try:
            cp_obj = Codigos_Postales.objects.get(
                codigo_postal=int(codigo_postal), estatus=1
            )
        ...
        # Buscar localidades
        localidades = (
            Localidades.objects.filter(catalogo_codigo_postal=cp_obj, estatus=1)
            .select_related(
                "catalogo_estado", "catalogo_municipio", "catalogo_tipo_asentamiento"
            )
            .order_by("localidad")
        )
        ...
        if not localidades.exists():
            data["message"] = (
                f"No hay localidades para el código postal {codigo_postal}"
            )
            return JsonResponse(data)
        ...
        for localidad in localidades:
            ...
            if localidad.catalogo_tipo_asentamiento:
                tipo_asentamiento = (
                    localidad.catalogo_tipo_asentamiento.tipo_asentamiento
                )
...
def validar_codigo_postal(request):
    ...
    try:
        ...
        tiene_localidades = Localidades.objects.filter(
            catalogo_codigo_postal=cp_obj, estatus=1
        ).exists()
...
def obtener_municipios_por_estado(request):
    ...
    if not estado_id:
        return JsonResponse({"success": False, "error": "ID de estado requerido"})
    ...
    try:
        ...
        return JsonResponse({"success": True, "municipios": list(municipios)})
...
def obtener_localidades_por_municipio(request):
    ...
    if not municipio_id:
        return JsonResponse({"success": False, "error": "ID de municipio requerido"})

    try:
        from .models import Localidades

        localidades = (
            Localidades.objects.filter(catalogo_municipio_id=municipio_id, estatus=1)
            .select_related("catalogo_tipo_asentamiento")
            .values("id", "localidad", "catalogo_tipo_asentamiento__tipo_asentamiento")
            .order_by("localidad")
        )
        ...
        return JsonResponse({"success": True, "localidades": localidades_data})
...
def actualizar_codigo_postal_ajax(request):
    ...
    try:
        ...
        if not codigo_postal:
            return JsonResponse(
                {"exito": False, "error": "El código postal es requerido"}, status=400
            )
        ...
        logger.info(
            f"Código postal actualizado para usuario {request.user.email}: {codigo_postal}"
        )
        ...
        return JsonResponse(
            {"exito": False, "error": "Datos JSON inválidos"}, status=400
        )

    except Exception as e:
        logger.error(f"Error en actualizar_codigo_postal_ajax: {str(e)}")
        return JsonResponse(
            {"exito": False, "error": "Error interno del servidor"}, status=500
        )
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                messages.error(
                    request, "Correo o contraseña incorrectos. Intenta nuevamente."
                )
...
class CrearEditarCVView(View):
    ...
    def get(self, request):
        ...
        curriculum, created = Curriculum.objects.get_or_create(
            interesado=interesado, defaults={"resumen_profesional": ""}
        )
...
def actualizar_foto_perfil_ajax(request):
    ...
    if not hasattr(request.user, "rol") or request.user.rol != "interesado":
        return JsonResponse(
            {"success": False, "error": f"Acceso denegado. Rol requerido: interesado"},
            status=403,
        )
    ...
    try:
        ...
        if "foto_perfil" not in request.FILES:
            return JsonResponse(
                {"success": False, "error": "No se recibió ninguna imagen"}, status=400
            )
        ...
        if not foto.name.lower().endswith((".jpg", ".jpeg")):
            return JsonResponse(
                {"success": False, "error": "Solo se permiten archivos JPG"}, status=400
            )
        ...
        try:
            ...
            return JsonResponse(
                {"success": False, "error": "Error al generar URL de la imagen"},
                status=500,
            )
...
def actualizar_perfil_ajax(request):
    ...
    try:
        ...
        interesado.apellido_paterno = request.POST.get(
            "apellido_paterno", interesado.apellido_paterno
        )
        interesado.apellido_materno = request.POST.get(
            "apellido_materno", interesado.apellido_materno
        )
        ...
        interesado.codigo_postal = request.POST.get(
            "codigo_postal", interesado.codigo_postal
        )
...
def actualizar_perfil_completo_ajax(request):
    ...
    try:
        ...
        interesado.estado_id = (
            int(estado_id) if estado_id and estado_id.isdigit() else None
        )
        interesado.municipio_id = (
            int(municipio_id) if municipio_id and municipio_id.isdigit() else None
        )
        interesado.localidad_id = (
            int(localidad_id) if localidad_id and localidad_id.isdigit() else None
        )

        # Nombres legibles
        interesado.estado_nombre = request.POST.get("estado_nombre", "").strip() or None
        interesado.municipio_nombre = (
            request.POST.get("municipio_nombre", "").strip() or None
        )
        interesado.localidad_nombre = (
            request.POST.get("localidad_nombre", "").strip() or None
        )
        ...
        if interesado.fecha_nacimiento:
            try:
                ...
                if hasattr(interesado.fecha_nacimiento, "strftime"):
                    fecha_nacimiento_formateada = interesado.fecha_nacimiento.strftime(
                        "%d/%m/%Y"
                    )
                else:
                    ...
                    fecha_obj = datetime.strptime(
                        str(interesado.fecha_nacimiento), "%Y-%m-%d"
                    )
...
def editar_experiencia_ajax(request, experiencia_id):
    ...
    try:
        ...
        experiencia = get_object_or_404(
            ExperienciaLaboral, id=experiencia_id, curriculum=curriculum
        )
...
def eliminar_experiencia_ajax(request, experiencia_id):
    ...
    try:
        if not hasattr(request.user, "interesado") or not hasattr(
            request.user.interesado, "curriculum"
        ):
            return JsonResponse(
                {"success": False, "error": "No tienes un curriculum creado."},
                status=404,
        ...
        try:
            experiencia = ExperienciaLaboral.objects.get(
                id=experiencia_id, curriculum=curriculum
            )
        ...
        logger.info(
            f"Experiencia {experiencia_id} eliminada exitosamente para usuario {request.user.email}"
        )
...
def eliminar_educacion_ajax(request, educacion_id):
    ...
    try:
        ...
        if not hasattr(request.user, "interesado"):
            return JsonResponse(
                {"success": False, "error": "Usuario no tiene perfil de interesado."},
                status=403,
            )
        ...
        # ✅ LOGGING PARA DEBUG
        logger.info(
            f"Intentando eliminar educación {educacion_id} para usuario {request.user.email}"
        )

        # Buscar la educación que pertenece al curriculum del usuario
        try:
            educacion = Educacion.objects.get(id=educacion_id, curriculum=curriculum)
        except Educacion.DoesNotExist:
            logger.warning(
                f"Educación {educacion_id} no encontrada para usuario {request.user.email}"
            )
            return JsonResponse(
                {
                    "success": False,
        ...
        # ✅ LOGGING DE ÉXITO
        logger.info(
            f"Educación {educacion_id} eliminada exitosamente para usuario {request.user.email}"
        )
        ...
        logger.error(
            f"Error eliminando educación {educacion_id} para usuario {request.user.email}: {str(e)}"
        )
        print(
            f"Error detallado en eliminar_educacion_ajax: {str(e)}"
...
def eliminar_idioma_ajax(request, idioma_id):
    ...
    try:
        ...
        try:
            idioma = IdiomaInteresado.objects.get(id=idioma_id, curriculum=curriculum)
        ...
        logger.info(
            f"Idioma {idioma_id} eliminado exitosamente para usuario {request.user.email}"
        )
...
def eliminar_idioma_ajax(request, idioma_id):
    ...
    try:
        curriculum = request.user.interesado.curriculum
        idioma = get_object_or_404(
            IdiomaInteresado, id=idioma_id, curriculum=curriculum
        )
        idioma.delete()

        return JsonResponse(
            {"success": True, "message": "Idioma eliminado exitosamente"}
        )
    except Exception as e:
        return JsonResponse({"success": False, "error": str(e)})
...
def descargar_cv_pdf(request):
    ...
    if interesado.foto_perfil and interesado.foto_perfil.name:
        try:
            ...
            ruta_completa = os.path.join(
                settings.MEDIA_ROOT, interesado.foto_perfil.name
            )
    ...
    html_string = render_to_string(
        "usuarios/cv_pdf_template.html", context, request=request
    )
    ...
    try:
        ...
        pdf_bytes = html_doc.write_pdf(optimize_images=True, presentational_hints=True)
    ...
    try:
        ...
        if not puede_ver_todas_las_vacantes(request.user):
            ...
            tiene_permiso = Postulacion.objects.filter(
                interesado=interesado, vacante__reclutador=request.user.reclutador
            ).exists()
        ...
        try:
            ...
            pdf_bytes = html_doc.write_pdf(
                optimize_images=True, presentational_hints=True
            )
...
class PublicarVacanteView(View):
    ...
    def get(self, request):
        ...
        if (
            not hasattr(request.user, "reclutador")
            or not request.user.reclutador.aprobado
        ):
            messages.error(
                request,
                "Tu cuenta de reclutador debe estar aprobada para publicar vacantes.",
            )
            return redirect("dashboard_reclutador")
    ...
    def post(self, request):
        ...
        if accion == "guardar_borrador":
            ...
            if not titulo:
                messages.error(
                    request,
                    "El título de la vacante es obligatorio, incluso para borradores.",
                )
            ...
            try:
                ...
                RequisitoVacante.objects.create(
                    vacante=vacante,
                    descripcion_requisitos=request.POST.get(
                ...
                messages.success(
                    request,
                    f'Borrador "{titulo}" guardado exitosamente. Puedes continuar editándolo después.',
                )
                return redirect("mis_vacantes")
            ...
            if vacante_form.is_valid() and requisito_form.is_valid():
                ...
                if errores:
                    messages.error(
                        request, f"Errores de validación: {', '.join(errores)}"
                    )
        ...
        try:
            ...
            if es_reclutador_admin(request.user):
                ...
                vacante = Vacante.objects.get(
                    id=vacante_id, reclutador=request.user.reclutador
                )
        except Vacante.DoesNotExist:
            messages.error(
                request, "Vacante no encontrada o no tienes permiso para editarla."
            )
            return redirect("mis_vacantes")

        # Obtener o crear requisitos si no existen
        requisito, created = RequisitoVacante.objects.get_or_create(
            vacante=vacante, defaults={"descripcion_requisitos": ""}
        )
    ...
    def post(self, request, vacante_id):
        ...
        if accion == "guardar_borrador":
            ...
            try:
                ...
                requisito.descripcion_requisitos = request.POST.get(
                    "descripcion_requisitos", ""
                )
                requisito.educacion_minima = request.POST.get("educacion_minima", "")
                requisito.experiencia_minima = (
                    request.POST.get("experiencia_minima") or None
                )
                requisito.save()
...
def index_view(request):
    ...
    vacantes_list = (
        ...
        .select_related("secretaria", "categoria", "reclutador")
    ...
    def post(self, request):
        ...
        try:
            with transaction.atomic():
                ...
                # Nombres legibles
                interesado.estado_nombre = (
                    request.POST.get("estado_nombre", "").strip() or None
                )
                interesado.municipio_nombre = (
                    request.POST.get("municipio_nombre", "").strip() or None
                )
                ...
                # Calle y número
                interesado.calle_numero = (
                    request.POST.get("calle_numero", "").strip() or None
                )
                ...
                curriculum.resumen_profesional = request.POST.get(
                    "resumen_profesional", ""
                ).strip()
                curriculum.save()
...
def autoguardar_informacion_personal(request):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "error": "Error al autoguardar información"}
        )
...
class DashboardReclutadorView(View):
    ...
    def get(self, request):
        ...
        if (
            ...
            messages.error(
                request,
                "Tu cuenta de reclutador debe estar aprobada para acceder al dashboard.",
            )
            return redirect("index")
...
def detalle_vacante_view(request, vacante_id):
    ...
    try:
        ...
        if vacante.estado_vacante == "borrador":
            ...
            if (
                not hasattr(request.user, "reclutador")
                or vacante.reclutador != request.user.reclutador
            ):
                messages.error(request, "No tienes permiso para ver esta vacante.")
                return redirect("mis_vacantes")
            ...
            if not vacante.aprobada:
                ...
                if (
                    ...
                    or request.user.rol != "reclutador"
                    or not hasattr(request.user, "reclutador")
                    or vacante.reclutador != request.user.reclutador
                ):
        ...
        if request.user.is_authenticated and request.user.rol == "interesado":
            ...
            if vacante.estado_vacante == "publicada" and vacante.aprobada:
                ya_postulado = Postulacion.objects.filter(
                    interesado=request.user.interesado, vacante=vacante
                ).exists()
        ...
        es_propietario = (
            ...
            and request.user.rol == "reclutador"
            and hasattr(request.user, "reclutador")
            and vacante.reclutador == request.user.reclutador
        )
...
def postularse_vacante(request, vacante_id):
    ...
    if request.user.rol != "interesado":
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse(
                {"success": False, "error": "Solo los interesados pueden postularse"}
            )
        else:
    ...
    try:
        ...
        vacante = get_object_or_404(
            Vacante, id=vacante_id, estado_vacante="publicada", aprobada=True
        )
        ...
        try:
            ...
            if (
                not interesado.apellido_paterno
                or not interesado.apellido_paterno.strip()
            ):
            ...
            if (
                not curriculum.resumen_profesional
                or not curriculum.resumen_profesional.strip()
            ):
...
def mis_postulaciones(request):
    ...
    postulaciones_list = (
        Postulacion.objects.filter(interesado=request.user.interesado)
        .select_related("vacante", "vacante__secretaria")
    ...
    context = {"postulaciones": page_obj, "page_obj": page_obj}
...
def retirar_postulacion(request, postulacion_id):
    ...
    try:
        ...
        postulacion = get_object_or_404(
            Postulacion, id=postulacion_id, interesado=request.user.interesado
        )
        ...
        # Mensaje de éxito
        success_msg = (
            f'Has retirado exitosamente tu postulación para "{vacante_titulo}"'
        )
...
def cerrar_vacante_ajax(request, vacante_id):
    ...
    if not hasattr(request.user, "reclutador") or not request.user.reclutador.aprobado:
        return JsonResponse(
            {"success": False, "error": "Tu cuenta de reclutador debe estar aprobada."},
            status=403,
        )
    ...
    try:
        ...
        if es_reclutador_admin(request.user):
            ...
            vacante = get_object_or_404(
                Vacante, id=vacante_id, reclutador=request.user.reclutador
            )
...
def reabrir_vacante_ajax(request, vacante_id):
    ...
    try:
        ...
        if fecha_extendida:
            mensaje_principal = (
                f'Vacante "{vacante.titulo}" reabierta exitosamente.{mensaje_fecha}'
            )
        else:
        ...
        # Log de la acción
        logger.info(
            f"Vacante {vacante_id} reabierta por reclutador {request.user.email}"
        )
        ...
        return JsonResponse(
            {
                ...
                "postulaciones_actuales": postulaciones_actuales,
                "limite_postulaciones": vacante.max_postulantes,
                "espacios_disponibles": vacante.max_postulantes
                ...
                "fecha_limite_formateada": nueva_fecha_limite.strftime("%d de %B de %Y")
                if nueva_fecha_limite
                else None,
            }
        )
...
def eliminar_borrador_ajax(request, vacante_id):
    ...
    try:
        ...
        # Log de la acción
        logger.info(
            f"Borrador {vacante_id} eliminado por reclutador {request.user.email}"
        )
...
def obtener_estado_vacante_ajax(request, vacante_id):
    ...
    try:
        ...
        puede_reabrir = (
            vacante.estado_vacante == "cerrada"
            and postulaciones_actuales < vacante.max_postulantes
        )
...
class VerPostulantesView(View):
    ...
    def get(self, request, vacante_id):
        ...
        try:
            ...
            messages.error(
                request, "Vacante no encontrada o no tienes permiso para verla."
            )
            return redirect("mis_vacantes")

        # Obtener todas las postulaciones para esta vacante
        postulaciones = (
            Postulacion.objects.filter(vacante=vacante)
            .select_related("interesado", "curriculum")
            .prefetch_related("curriculum__habilidades__habilidad")
            .order_by("-fecha_postulacion")
        )
    ...
    def _calcular_estadisticas(self, postulaciones):
        ...
        # Contar nuevos hoy
        nuevos_hoy = postulaciones.filter(fecha_postulacion__date=date.today()).count()
...
def cambiar_estado_postulacion(request, postulacion_id):
    ...
    if request.user.rol != "reclutador":
        logger.warning(
            f"Usuario {request.user.email} no es reclutador. Rol: {request.user.rol}"
        )
        return JsonResponse(
    ...
    if not hasattr(request.user, "reclutador"):
        ...
        return JsonResponse(
            {"success": False, "error": "No tienes perfil de reclutador asociado"},
            status=403,
        )
    ...
    try:
        ...
        print(
            f"Reclutador {request.user.email} cambió estado de postulación {postulacion_id} "
...
def ver_perfil_candidato(request, interesado_id):
    ...
    try:
        ...
        interesado = get_object_or_404(
            Interesado.objects.select_related("usuario"), id=interesado_id
        )
...
def busqueda_vacantes_ajax(request):
    ...
    vacantes = Vacante.objects.filter(
        estado_vacante="publicada", aprobada=True
...
def verificar_estado_cv_ajax(request):
    ...
    try:
        ...
        return JsonResponse(
            {"cv_completo": es_completo, "mensaje": mensaje, "success": True}
        )

    except AttributeError:
        return JsonResponse(
            {"error": "Usuario no es un interesado", "cv_completo": False}, status=400
        )

    except Exception as e:
        return JsonResponse({"error": str(e), "cv_completo": False}, status=500)
